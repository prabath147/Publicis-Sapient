...
import com.admin.service.client.auth.AuthClient;
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
                ...
                // UserMasterImpl userMaster = webClientBuilder.build().get().uri("http://localhost:8081/api/auth/valid-token").header(HttpHeaders.AUTHORIZATION, "Bearer " + jwt).retrieve().bodyToMono(UserMasterImpl.class).block();
